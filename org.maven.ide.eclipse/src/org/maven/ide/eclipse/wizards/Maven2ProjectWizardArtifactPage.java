 ...
 * and the directories to create.
 *
 * This wizard page gathers Maven2 specific information. The user must specify
 * the necessary information about the Maven2 artifact and she can also decide
 * which directories of the default Maven2 directory structure should be created.
 * Input validation is performed in order to make sure that all the provided
 * information is valid before letting the wizard continue.
 */
public class Maven2ProjectWizardArtifactPage extends WizardPage {
  ...
  private Maven2ArtifactComponent artifactComponent;
   ...
   */
  public Maven2ProjectWizardArtifactPage() {
    super( "Maven2ProjectWizardArtifactPage" );
   ...
   * {@inheritDoc}
   *
   * This wizard page contains a <code>Maven2ArtifactComponent</code> to gather
   * information about the Maven2 artifact and a <code>Maven2DirectoriesComponent</code>
   * which allows to choose which directories of the default Maven2 directory
   * structure to create.
   */
   ...
   * the user.
   *
   * @return  The Maven2 model containing the provided artifact information.
  ...
  /**
   * Returns the directories of the default Maven2 directory structure selected
   * by the user.
   *
   * These directories should be created along with the new project.
   *
   * @return  The Maven2 directories selected by the user.
  ...
  /**
   * Returns all the required source classpath entries for the Maven2 directories
   * currently selected by the user.
   *
   * <p>
   * Note that only source and source related resource directories require such
   * a classpath entry since the contents of other directories are not relevant
   * to Eclipse. More concretely, only the following Maven2 directories require
   * a classpath entry to be created:
   * <ul>
   *   <li>src/main/java</li>
   ...
   * </p>
   * <p>
   * The output folders of the classpath entries are thereby set up to match
   * the output folders used by Maven2.
   * </p>
   * <p>
   * Note that the output folder for some classpath entry is only explicitly
   * set if it does not correspond to the default output path as returned by
   * <code>getDefaultOutputLocationPath( IPath projectPath )</code> as this
   * is implicitly inferred.
   * </p>
   *
   * @param projectPath  The path to the project for which the classpath entries
   *                     are built.
   * @return             All the required source classpath entries for the
   *                     Maven2 directories currently selected by the user.
   *                     Neither the array nor any of its elements is
   *                     <code>null</code> and all the classpath entries are of
   *                     type <code>JavaCore.CPE_SOURCE</code>.
   */
  ...
  /**
   * Returns a single source classpath entry for the given Maven2
   * <code>directory</code>.
   *
   * The source classpath entry will have the appropriate output folder set
   * as defined by Maven2.
   *
   * @param projectPath  The path to the project for which the classpath entries
   *                     are built.
   * @param directory    The Maven2 directory for which to create the source
   *                     classpath entry.
   * @return             The source classpath entry of type
  ...
  private static IClasspathEntry getClasspathEntry( IPath projectPath, Maven2Directory directory ) {
    ...
    if ( directory.getOutputPath() != null
         && !DEFAULT_MAVEN2_OUTPUT_LOCATION.equals( directory.getOutputPath() ) ) {
      outputPath = projectPath.append( directory.getOutputPath() );
    }

    return JavaCore.newSourceEntry(
        projectPath.append( directory.getPath() ),
        new IPath[0],
        outputPath );
  }

  /**
   * Returns the default output location path of the project.
   *
   * This path is intended as the default output location path in the Java
   * .classpath file.
   *
   * @param projectPath  The path to the project being created.
   * @return             The project relative path to the default ouput location.
   *                     Is never <code>null</code>.
   */
